...
- `poller.py live` is a separate long-running poller process — polls Upstox every 5s during market hours, computes Supertrend/EMA/RSI, fires Telegram alerts, tracks trades in SQLite via `db/queries.py`. (Note: the top-level `backend/poller.py` is the CLI dispatcher; the actual loop logic lives in `live/poller.py` — same base filename, different files.)
...
Hard rule: **never start `server.py`, `trade_server.py`, or `poller.py live`** yourself — these are run by the user in their own terminals. You may read logs/DB state, but do not launch the server or poller as a side effect of testing a change.
