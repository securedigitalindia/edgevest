...
**Option-chain triggers → draft trades (2026-09-22).** The per-tick alert triggers above were removed from `config.TRIGGERS`; the first option-chain triggers (`config.CHAIN_TRIGGERS`, evaluated on each 5-minute option-chain snapshot) create *draft* recommendations — never live ones — for an admin to review and publish: three 1:2 NIFTY CE calendar/diagonal setups today (`docs/prd/option-chain-calendar-ratio-trigger.md`).

**Admin strategy research.** An admin-only Strategies section (`/profile/strategies`) lists backtested strategies — the PE+CE ratio diagonal and the PE+CE 1:2 calendar ratio — and shows per-window results for admin-set inputs (`docs/prd/admin-strategies-dashboard.md`, `docs/prd/pe-ce-ratio-spread-1x2.md`). Research/monitoring only; not client-visible and not a trading signal.
