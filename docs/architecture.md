...
- **`chain_triggers.py`** — option-chain triggers (`config.CHAIN_TRIGGERS`), evaluated once per 5-min `option_chain_5m` snapshot right after `option_chain_capture.run_capture()` in the poller loop; on fire they create a **draft** trade via `add_manual_trade(status="draft")` (no margin until published) and send a Telegram "Trigger Fired" alert saying whether a draft is linked. Two types today: `calendar_ratio_credit` (six triggers, one shared `config._calendar_ratio_trigger()` template swept across `itm_points` 400/300/200/100/0/-100, ITM 400 down to OTM 100) and `pe_ratio_diagonal_credit` (the 4-leg 1:2:1:2 PE ratio diagonal, `NIFTY_PE_RATIO_DIAG_4LEG_400` — reuses the existing backtested strategy's own expiry/strike resolution). Front-month NIFTY future price is read live from Upstox, not from the DB; one draft per trigger per side per IST day, enforced by the `chain_trigger_fires` table — design in `docs/prd/option-chain-calendar-ratio-trigger.md`. The poller polls every `SYMBOLS` entry even with no per-tick triggers (`_build_all_triggers()`), so an empty `TRIGGERS` does not stop ticks/candles/price cache/option-chain capture/EOD sync.
...
- **Routing**: React Router v7 — `/` → `/dashboard`, `/dashboard`, `/trades`, `/games`, `/games/:id`, catch-all → `/dashboard`.
...
- **Screens**: `Dashboard` (overview home — active-trades count, monthly report summary, refer-and-earn promo, games teaser), `Trades` (recommendations, P&L, instrument search — formerly the Dashboard screen), `Games` (paper-trading games list) + `GameDetail`, `Landing`, `SetupWizard`.
...
**Frontend deploys are versioned, not a flat overwrite** (`frontend/deploy/deploy.sh`, added `v5.0`): each deploy builds and uploads `dist/` to its own S3 prefix (`s3://<bucket>/releases/<release-id>/`, `release-id` defaulting to `git describe --tags --always --dirty`), then repoints that environment's CloudFront distribution's `OriginPath` at the new prefix and invalidates `/*`. Nothing previously deployed is ever overwritten, so rollback is just repointing `OriginPath` at an older release (`deploy.sh rollback <env> <release-id>`) — no rebuild, no reupload. All three buckets were migrated onto this pattern at `v5.0`; the old flat-root deploy layout (loose `index.html`/`assets/`/`icons/` at the bucket root) was retired and deleted from all three at the same time. See `docs/release-checklist.md` for the full release procedure.
...
- **`staging` has no backend yet, even though its frontend is now live** — `staging.edgevest.in` serves a real build (`releases/v5.0`, same pattern as prod/dev) as of `v5.0`, but there's still no backend server behind it — API calls there have nowhere to go until one is stood up.
