...
| GET | `/api/subscriptions` | `require_role(super_admin, admin)` | — | `{ subscriptions: [...] }` — each row also carries `user_id` and `plan_gem_cost` (the plan's *current* `gem_cost`, joined live — not a value snapshotted at subscription time) so the frontend can label a `amount_paid == 0` row as gem-redeemed vs free, and cross-link to `/api/payments` by `user_id`. |
