...
No other read paths change. `margin_required`/`margin_final` keep their exact current behavior and every existing caller (Dashboard.jsx, `/api/recommendations`) is unaffected. Rows created before this migration ships will have `margin_at_entry IS NULL` forever (no reconstruction is possible — the pre-migration overwrite already happened, if it happened) — any month whose report includes such a row treats its margin *contribution* as 0/excluded from the day-by-day sum, but the row itself still counts toward `margin_positions`/`totalPositions`/Booked-Open/New-Carried (fixed 2026-09; see the roll-day exception note under "Margin" for the exact bug this caused when it was still fully excluded). Call the margin-figure gap out as a known, permanent data gap for pre-migration history rather than something to solve for.
...
**NULL-margin rows still count toward `margin_positions`, added 2026-09-01:** the "contribute 0" language above used to mean skipped from `margin_positions` entirely — that broke the `totalPositions = Booked + Open` invariant, since `bookedCount` (from the separate `pnl_events` query) has no `margin_at_entry` filter and would subtract a trade that was never added to `totalPositions` in the first place. Symptom that surfaced this in prod: an August report showed 6 open at month-end but 8 still open today — impossible under the invariant, since "still open today" is a strict subset of "open at month-end." Fix: a NULL-margin row is now included in `margin_positions` (and counts toward `new_position_count`/`carried_position_count`, and the Booked/Open split) with `margin_at_entry: null` in the API response (rendered as "—", not "₹0" — `fmtRs(null)` already does this) — it's excluded only from the day-by-day margin *sum/peak/avg* (the `intervals` list), where there genuinely is no number to contribute. `margin_positions.sort()` treats a `null` margin_at_entry as lowest (sorts last), never crashes comparing it to a number.
